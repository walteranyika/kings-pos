	...
	<form method="post" id="app">
		...
		<div class="form-horizontal" style="margin-top: 20px;">
		    ...
		    <div class="col-md-5">
		      	...
		      	>
		      	<option value="null">Please Select a Invoice First</option>
		      	@foreach($invoices as $invoice)
		      		<option value="{{$invoice->reference_no}}">
    	...
    	<div>
    		<table class="table table-bordered bg-sells">
				...
				<tfoot>
					...
					<tr v-if="is_return_balance">
						<td colspan="{{colSpanNumber()}}" @if(!rtlLocale()) style="text-align: right; font-weight: bold;" @endif>		
							Return Balance
						</td>

						<td style="font-size: 20px; font-weight: bold; text-align: justify; color: #f05f26;">
							&#2547; @{{this.previous_balance}}
						</td>
					</tr>

					<tr class="bg-light-skyblue">
						<td colspan="{{colSpanNumber()}}" @if(!rtlLocale()) style="text-align: right; font-weight: bold;" @endif>		
							Total Payable
						</td>
						<td colspan="2" style="font-size: 20px; font-weight: bold; text-align: justify; color: green;">
							&#2547; @{{netTotal}} 
						</td>
					</tr>
    ...
    <script>
    	...
    	var app = new Vue({
		    ...
		    computed: {
		    	...
		    	netTotal: function () {
		    		return (parseFloat(this.total) - parseFloat(this.previous_balance) + parseFloat(this.invoice_tax) + parseInt(this.shipping_cost) - parseFloat(this.discountAmount)).toFixed(2)
		    	},
		    ...
		    methods:{
		        ...
		        searchInvoice: function (){
		        	var self = this
		        	axios.get('/admin/return/search/invoice/' + self.invoice_no)
			        .then(function (response) {
			          console.log(response.data)
			          self.previous_balance = response.data
			          if(self.previous_balance > 0){
			          	self.is_return_balance = true
			          }else{
			          	self.is_return_balance = false
			          }
